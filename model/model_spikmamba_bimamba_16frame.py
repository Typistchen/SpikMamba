...
class Spikformer(nn.Module):
    ...
    def forward_features(self, x, inference_params=None):
        ...
        residual = None
        hidden_states = x.reshape(self.batch_size, -1, self.embed_dims) # 32,2048,256
        ...
        hidden_states = hidden_states.view(int(self.batch_size),x.size(1),-1,self.embed_dims)
...
if __name__ == '__main__':
    ...
    model = Spikformer(
        ...
        in_channels=3, num_classes=300, qkv_bias=True, num_frames=16,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=2, sr_ratios=1, batch_size = 16
        # **kwargs
    ).cuda()
    ...
    a = torch.rand(16, 3, 16, 128, 128).cuda()
    out = model.forward(a)
