...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, kernel_size=1, in_chans=3, embed_dim=768, num_frames = 8):
        ...
        self.proj_bn = nn.BatchNorm1d(int(embed_dim * num_frames / 2 ))
        self.proj_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.reshape(b, n, num_heads, head_dim).permute(0, 2, 1, 3)  # l:1024 num_heads:8 head_dim = 32
...
class Spikformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = x
        x = x.view(int(x.size(0)/8),x.size(1),-1,self.embed_dims)
...
@register_model
def spikmamba_sla_8(pretrained=False, **kwargs):
    model = Spikformer(
...
if __name__ == '__main__':
    ...
    model = Spikformer(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=2, sr_ratios=1, batch_size = 1
        # **kwargs
    ).to(device)
    # a = torch.rand(1, 3, 8, 128, 128).to(device)
    # model = torch.load('/home/dsz/Documents/eventcamera/spikformer/cifar10dvs/complete_model_hardvs94.06900396432111.pth')


    data = np.load("/home/dsz/Documents/eventcamera/ExACT/event_copy_2_npz/001_event/01-s-b-s3/01-s-b-s3.npz",allow_pickle=True)
    events = data['images']
    if events.ndim < 4:
    ...
    print(events_data.shape)
    # 加载预训练模型的参数
    pretrained_dict = torch.load('/home/dsz/Documents/eventcamera/spikformer/cifar10dvs/complete_model_hardvs94.06900396432111.pth')
    torch.save(pretrained_dict.state_dict(), 'tttttttttt.pth')
    pretrained_dict = torch.load('tttttttttt.pth')
    # 获取新模型的参数字典
    ...
    print('v',v.size())
    print('z',atten.size())
    atten = atten[0].unsqueeze(0)
    atten = atten.cpu().detach().numpy()


    image = Image.open('/home/dsz/Documents/eventcamera/ExACT/event/001_event/01-s-b-s3/0.png')

# visualize_grid_to_grid_with_cls(atten[3][0,0,1:,1:], 105, image)

    visualize_grid_to_grid(atten[0,0,:,:], 100, image)