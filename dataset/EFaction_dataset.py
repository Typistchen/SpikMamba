...
class PAF(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # label
        label_str = event_stream_path.split('/')[8]
        # label_str = event_stream_path.split('\\')[3]
        label_idx = 0

        ts, x, y, pol = self.getDVSeventsDavis(event_stream_path)
        events_stream = np.array([x,y,ts,pol]).transpose()

        all_frame = self.adaptive_event_sampling(events_stream, event_stream_path)
        ...
        with AedatFile(file) as f:
            ...
            for event in f['events']:
                if first_timestamp is None:
                    first_timestamp = event.timestamp / 1e6  # 将时间戳转换为秒
                # 将绝对时间转换为相对时间（秒为单位）
                relative_time = (event.timestamp / 1e6) - first_timestamp
                # 将事件极性转换为0和1表示
                polarity = int(event.polarity)
                # 将事件数据以't x y p'的格式写入输出文件
                event_str = f"{relative_time} {event.x} {event.y} {polarity}\n"
        # print('Total number of DVS events returned =', len(ts))
        return relative_time, event.x, event.y, polarity
    ...
    def generate_abs_image(self, events_clip,file):
        """
    ...
    def adaptive_event_sampling(self, events_stream, file):

        
        N, _  = events_stream.shape
        divide_N = int(np.floor(N / 2))

        if self.if_sufficiently_sampled(events_stream, file): # return True for sufficiently sampled, no need for proceed.
        ...
        frame_list1 = self.adaptive_event_sampling(events_stream[:divide_N, :], file)
        frame_list2 = self.adaptive_event_sampling(events_stream[divide_N:, :], file)
        return np.concatenate((frame_list1, frame_list2),axis=0)

    def if_sufficiently_sampled(self, events_stream, file):
        
        N, _ = events_stream.shape
        # print(N)
        half_N = int(np.floor(N / 2))
        # print(half_N)
        half_frame1 = self.generate_abs_image(events_stream[:half_N, :],file)
        # print(half_frame1.sum())
        half_frame2 = self.generate_abs_image(events_stream[half_N:, :],file)
...
if __name__ == '__main__':
    all_path = r'/home/dsz/Documents/eventcamera/ExACT/efaction_file_names.txt' # TODO: Change to your directory
    num_events = 80000 # 1093470 546735 273367 136683 68341
    median_length = 1500000
    frame = 1513
    tf = open("/home/dsz/Documents/eventcamera/ExACT/EFaction.json", "r")
    ...
    for step, (events_image, class_idxs, event_stream_path) in enumerate(feeder):
        ...
        for i in range(B):
            for j in range(T):
                file_path = event_stream_path[0].replace('PAF', 'PAF_RGB_Sampled').replace('.aedat4', '')
